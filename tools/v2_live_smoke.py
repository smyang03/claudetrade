    ...
    *,
    ...
    with TemporaryDirectory() as tmp:
        ...
        snapshot = snapshot_store.create_snapshot(
            prompt_version=DEFAULT_V2_CONFIG.prompt_version,
            market=market,
        ...
        sizing = FixedSizer(DEFAULT_V2_CONFIG).size(
            market=market,
            price_krw=price_krw,
            usd_krw=profile.usd_krw,
            cash_krw=manager.cash,
        )
        safety = SafetyGate(DEFAULT_V2_CONFIG).evaluate(
            SafetyContext(
                market=market,
                runtime_mode=runtime_mode,
                ticker=ticker,
...
def main() -> int:
    ...
    results = [
        run_live_smoke(market=market, runtime_mode=args.runtime_mode, usd_krw=args.usd_krw)
    ...
    if args.json:
        ...
        for item in results:
            ...
            print(f"{item.get('market')} {item.get('runtime_mode')} smoke: {status}")
            if not item.get("ok"):
