...
== 판정 (2026-08-26 새벽 실측 — 섹터맵 486→695 확장 후 재검) ==
- 축2 비정상 거래량: **보류/기각 방향** — 전체 −0.10%p 무변별, 월별 부호 불일치.
  문헌(ACG 2006)의 경고("계약비용보다 이익이 작다")와 오히려 정합.
- 축1 섹터 단독성: **문헌 예측의 정반대가 일관되게 나옴** — 전체 −2.35%p,
  07/08월 부호 재현(−12.9/−0.9), 밴드 내 −6.78%p(단독쪽 단독 t −2.9).
  즉 우리 급락 풀에서는 **섹터 동반 급락(패닉의 파편=비정보성)이 반등이 강하고,
  섹터 대비 단독 급락(고유 악재 가능성)이 나쁘다.** 뉴스/무뉴스 문헌의 정신과는
  정합하는 해석이나, **사전등록 방향의 역이므로 이 표본으로 확정 불가(사후 발견).**
  → "섹터 동반성" 신규 가설로 등록: 판정은 30건 시점 forward 재계산(이 스크립트
  재실행)으로 하고, 그때 같은 부호+t>=2면 밴드 내 필터(단독 급락 배제) 승인 요청.
  그 전 적용 금지.
"""
