...
CREATE INDEX IF NOT EXISTS idx_fact_market_judgment_session
    ON fact_market_judgment(market, session_date, parse_status);
"""
...
def parse_market_judgment_file(path: str | Path, *, now: str | None = None) -> dict[str, Any]:
    file_path = Path(path)
    ...
    try:
        ...
        return {
            "market_key": f"{session_date}:{market}",
    ...
    return {
        ...
        "source_refs_json": _json({"path": str(file_path), "file_name": file_path.name}),
    }
...
def build_market_judgment_facts(
    *,
    ...
    rows = [parse_market_judgment_file(path, now=started) for path in files]
