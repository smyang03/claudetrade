...
## 8. 과거 시장데이터 리플레이 (2026-07-14 02:20 추가) — 새 게이트의 7월 반사실

7월 judge 원시 응답 244건(`logs/raw_calls/`) 전수 파싱 + 분봉 인과 리플레이(`tools/rr_reject_causal_replay.replay`, no-lookahead·존상단 보수체결·봉내 손절우선).

**RR 밴드 분포 (consistent 분모):**

| 밴드 | KR (플랜 42) | US (플랜 65) |
|---|---:|---:|
| <1.1 | 2 | 0 |
| 1.1~1.5 | **11** | 5 |
| ≥1.5 | 29 | 60 |

- **US: RR은 병목 아님** — judge 산출의 92%가 이미 1.5 이상. 기존 SAFETY_BLOCKED 거부 코호트(58건, <1.5)는 인과 리플레이 **−1.99%/승률 22.6%** → **US 1.5 유지가 옳음을 과거 데이터가 재확인**.
- **KR: 1.1 완화로 새로 열리는 밴드 = 7월 11건**(생산 +38%). 그런데 이 11건의 인과 리플레이:

| 출구 | mean | median | 승률 |
|---|---:|---:|---:|
| 원목표(1.0x) | **−1.45%** | −2.86% | 27% |
| 조기익절 0.5x | −1.11% | −2.33% | 45% |
| 조기익절 0.4x | **−0.27%** | **+1.15%** | **64%** |
| 조기익절 0.33x | −0.46% | +0.91% | 64% |

**판정**: [[entry-pipeline-collapse-rr-gate]] 처방("RR 재캘리 + 조기익절 tier는 한 세트, 하나만 하면 손실 재개")이 실데이터로 재확인됐다. KR 1.1 밴드는 원목표 출구로는 순손실 방향이고, 조기익절 0.4x가 승률 27%→64%로 뒤집지만 좌측꼬리(손절 −4~−5.5%) 때문에 이 소표본에선 mean이 아직 살짝 음수다.

한계: n=11·7월 단월. 리플레이는 원시 stop/target만 모델링 — 라이브의 Claude 출구 관리(검증된 강점, TARGET +107)가 반영 안 돼 비관 편향 가능.

**운영자 선택지**:
- A. 현행 유지(KR 1.1 라이브) + **조기익절 tier 배선 즉시 착수** — 밴드를 살리고 출구로 방어 (처방의 "한 세트" 완성)
- B. 조기익절 배선 완료까지 KR 1.1 → 1.5 임시 환원 — 손실 방향 밴드 차단, 대신 플랜 생산 −38%

(어느 쪽도 판정 파라미터 변경이므로 실행은 운영자 승인 후.)
