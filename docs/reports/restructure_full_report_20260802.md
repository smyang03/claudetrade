...
## 7. 정정·명확화 (2026-08-02, 외부 검토 반영)

### 7-1. 전략 정체성 — 명칭을 정확히 한다
> **운영 배선과 DB 축적 구조는 정상이다. 그러나 현재 매수 레인은 "저평가 장기보유"가 아니라 "급락 반등 예측형 5일 전략"이다. 저평가 장기보유로의 전략 전환은 아직 코드와 검증에 반영되지 않았다.**

이 문서와 앞선 보고에서 쓴 "저점을 골라 상승까지 기다렸다 판다"는 표현은 부정확했다. 구현된 실체는:
- day_losers는 **저평가 판정이 아니라 당일 낙폭 스크린**이다 (밸류에이션 정보 없음)
- GBM 확률·예측 net은 **단기 반등 예측 모델**이다
- TP +12% / SL −25% / **최대 5거래일** — 무기한 보유가 아니라 **한정된 기간의 반등 대기**다
- 즉 본질은 **단기 mean-reversion**이며, 그렇게 부르고 그렇게 평가해야 한다

진짜 "저평가 매수 후 상승까지 보유"로 가려면 별도 전략 정의가 필요하다: 저평가 정의(PER/PBR/FCF/동종 대비 할인), 상승 촉매, 목표가 범위, 보유 기간, 논리 무효화 조건, 손실 제한·최대 노출, 과거·forward 검증. **현재 이 중 어느 것도 구현·검증되지 않았다.**

### 7-2. 코어 3종목 매도 계약 — 코드로 확정한 사실
`trading_bot.py`의 isolated 처리에서 코어 소스 포지션은 **tp/sl/tp_pct/sl_pct가 0으로 강제**되고 `exit_contract="strategy_rebalance_only"`로 설정된다. 즉:

| 항목 | 실체 |
|---|---|
| 보유 목적 | 운영자 승격 장기보유 (2026-07-25 결정) |
| TP/SL/트레일링 | **없음 (코드가 0으로 강제)** |
| 일반 자동매도(advisor) | 적용 안 됨 (isolated) |
| 전략 리밸런스 매도 | **현재 정지** — 재구성에서 `PROFIT_STRATEGY_ENABLED_IDS`를 비워 유일한 자동 청산 경로도 꺼졌다 |
| **유일한 청산 경로** | **운영자 수동 매도뿐** |
| 손실 제한 / 강제 청산 | **없음** |
| 현재 평가 | 275280 −10.5% / 275300 −9.3% / SCHG −2.5% (8/1 기준) |

⚠️ 재구성 부작용: ids를 비우기 전에는 월별 리밸런스 신호가 자동 청산 경로였으나 지금은 그것도 없다. 코어 손실이 커져도 시스템은 아무것도 하지 않는다 — **손실 한도(예: −20% 도달 시 운영자 알림)라도 둘지는 운영자 결정 항목**이다.

### 7-3. DB 축적의 정확한 해석
- 스크리너·후보·선정·judge·audit DB: 계속 축적 (차단은 주문 계층에만)
- 다만 매수 차단으로 **기존 레인의 주문·실현손익·청산 outcome 데이터는 더 이상 발생하지 않는다** (swing 레인 주문 데이터만 발생)
- **월요일 확인 항목**: ① 분석 데이터 증가 + 주문 0건 (`LEGACY_BUY_DISABLED` 로그) ② haiku 전환 후 judge/consensus의 **판단 분포·품질 비교** (호출량이 아니라 라벨 분포 — BUY_READY 비율, consensus_mode 분포를 8/2 전후로 대조)
