# Developed Work Summary - 2026-05-05

이 문서는 완료된 작업의 요약만 보존합니다. 2026-05-05 정리에서 완료된 세부 plan 문서는 `docs/plans/`에서 삭제했고, 필요하면 Git history에서 복구합니다. 현재 실행해야 할 일은 [TODO_ROADMAP.md](TODO_ROADMAP.md)에만 남깁니다.
...
삭제하지 않은 P0 PathB QA 문서 2개는 현재 `tests/test_pathb_runtime.py` 실패가 있어 active로 남겼습니다.
...
- PathB post-isolation / fill-dashboard P0 QA: 구현 항목은 있으나 현재 focused QA가 실패합니다.
- KR/US live ops QA: guardian가 아직 `BLOCK_START`입니다.
- RiskManager KR/US 분리: 아직 단일 `self.risk` 구조입니다.
- Preopen/extended-hours 성능 판단: 구현은 완료됐지만 5~10세션 관찰 리포트가 필요합니다.
