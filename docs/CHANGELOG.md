...
- **무엇:** `v2_daily_loop`(forward 측정+sync 오케스트레이터)를 재실행(`--market ALL --forward-lookback-days 20 --skip-simulation --skip-optimizer`). dry-run 검증 후 실제 실행. FORWARD_MEASURED 이벤트 169→389(+220), `v2_learning_performance.forward_complete` 6월 0→42.
...
- **현재상태:** 백로그 catch-up 완료(완료가능 6/1~6/12 중 42/68 complete, 최근 42건은 forward 미경과 정상 pending). event store WAL+30s라 US장중 봇과 경합 안전. 코드변경 없음(죽은 잡 재실행).
- **롤백조건:** 해당 없음(측정 데이터 기록, 주문/브로커 무관).
- **잔여:** ① 완료가능인데 미완료 26건(partial-horizon/sync join 의심) 추가 진단. ② **잡 재정지 방지 모니터 부재** = #2 상시 정합성 체크의 1순위 근거. ③ catch-up 후 lesson forward-validation "0 valid lessons" 재평가 필요.
- **커밋:** 데이터 조치(미커밋 코드 없음).
