...
### 트렌드 방어 오버레이 진입 게이트 (shadow, 기본 off)

- **무엇:** PathB 진입 스캔에 지수 추세 방어 게이트 신규(`bot/trend_overlay_gate.py` + `tools/refresh_trend_overlay_signal.py` + `runtime/pathb_runtime.py::_pathb_trend_overlay_gate`). 신규 env `TREND_OVERLAY_GATE_MODE`(off/shadow/enforce, **기본 off**). 지수 월말종가 < 10mo SMA(하락추세)면 신규 진입 축소.
- **왜:** 오늘 손실의 진짜 원인 = 하락추세 베타 giveback(QQQ 천장서 −1.3% 반납). 능동 return-알파는 5종 백테스트로 전멸 확정(trailing·분할익절·다이아핸드(target까지보유 −111.5%p)·시장필터(6월 단일월 과적합)·conviction(승자가 더 낮음)). 유일 생존 = risk-알파 트렌드 방어(지수 30년 검증: MaxDD 반토막·Sharpe↑·CAGR 거의유지). 방향 전환의 첫 부품.
- **현재상태:** **기본 off = 완전 no-op, 라이브 무영향.** 현재 US·KR 둘 다 상승추세(SPY 733>SMA691, KOSPI 8930>5620)라 enforce여도 무동작. broker-truth fail-closed 비접촉, 청산/보유 무관, fail-open(신호 결손/stale 미차단). 153 passed, preflight ok=True. 커밋 `efaf0fc`.
- **롤백조건:** off 기본이라 롤백 불필요. shadow/enforce는 운영자가 명시적으로 켤 때만. active 책 forward 검증(지수레벨≠개별주 진입) 실패 시 off 영구 유지.
- **잔여:** shadow 가동(봇 재시작 + 토글 shadow + refresh 도구 일1회 cron)은 운영 머신 launch라 운영자 직접. (기각) 능동 매도/시장필터 개선 5종 — 백테스트 전멸, 재시도 시 오늘 도구들 가리킬 것.

## 2026-06-23

### 현실 직시 기록 — 시스템 수익성 AI 본심 (운영자 요청, 문서만)

- **무엇:** `docs/important/HONEST_ASSESSMENT_20260623.md` 신규 — 운영자가 "헸던 말 반복 말고 본심 말하라"고 물어 나온 솔직 평가를 포장 없이 기록.
- **왜:** 다음 세션이 또 "개선 아이디어 리스트"로 핵심 질문을 회피하지 않게. 핵심: AI는 새 알파 아이디어 없고(데이터 7번 무엣지), 청산튜닝/비용절감은 진짜 질문 회피였으며, 회피한 질문 = **"이 시스템이 수수료+환전 이기고 돈 버는 게 맞나, 아니면 실탄 빼야 하나"**. +71% capture는 5월 강세·소표본 안전담요. CLAUDE.md "작동 안 하면 인정이 올바른 결정" 원칙 적용을 미뤄왔음.
- **현재상태:** 문서만. 코드/config/토글/실탄 무변경. **규율: "비용 이기는가"라는 단일 질문에 정직히 답(한 사이클 net·FX 반영·수수료 차감)하기 전엔 실탄 확대 금지.**
- **롤백조건:** 해당 없음(기록).
- **잔여:** 다음 본작업 후보 = 개선 튜닝이 아니라 **FX 반영 + 전 사이클 net으로 "비용 이기는가" 정직 결산.**

- **무엇:** TODO_ROADMAP [A]·#4a 항목 갱신. 코드/config/토글 무변경(문서만).
- **왜:** valid=0의 원인 분석 결과 — 자동 교훈은 사후 내러티브(hindsight)이고 전부 진입(watch_only 풀기)에 관한 건데 진입 무엣지라 재사용할 엣지가 없음. 검증 결과 US=비용미달(marginal)·KR=손실(invalid_block)·노이즈(KR +11/8건). 운영자 원래 의도("장종료 적절교훈 재사용")가 이 구조로 성립 안 함 확인.
- **현재상태:** ① forward-validation 레이어 = "이기는 교훈 발굴기" 아니라 **"진실 필터/가드"**로 기대 재정의(유지·죽이지 않음, valid 0이 정상). ② **#4a = 🕒 장기관찰**(빨라야 2~3개월, KR 일부 셀 영영 미달; min_wo=30 미스칼리브레이션이나 보유 반사실 분산 극심 −21~+25%라 임계 안 낮춤; 단기 기대 접음). 행동 검증은 자동교훈 아니라 shadow 실험(flow 게이트 등)으로.
- **롤백조건:** 없음(문서/기대 조정). #4a verdict는 수개월 후 재검토.
