...
- PathB miss quality shows US `INVALID_PRICE` cancel rows `n=29`, `zone_reentered=26`, `avg_mfe_30m=+1.222%`; this is a P0 diagnostics/reporting item, not a broker-truth or sizing-policy relaxation item.
...
| KR `NO_SIGNAL` / ORP timing report | Aggregate recent `trade_ready -> signal_fired=0` rows by strategy/date/source and join ORP trade_ready rows to intraday ORP block reasons before any KR threshold/window change. | KR no-signal is classified as selection timing, strategy condition, evidence quality, or risk/order/broker gate; shared strategy changes include US PathB impact review. |
| PathB `INVALID_PRICE` miss diagnostics | Report PathB cancel reason, zone reentry, 30m MFE/MAE, price source, stale quote, tick-size/native/KRW conversion, and order timing for `INVALID_PRICE` misses. | The cause of `INVALID_PRICE` misses is visible without weakening broker-truth fail-closed, sizing reason split, slippage cap, or order submit policy. |
