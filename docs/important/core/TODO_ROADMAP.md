...
| hold advisor 정식 lesson_validation 편입 (#4a, **구현 완료** 커밋 `8ae6fc8`) | profit_guard 청산 counterfactual(SELL 실현 vs HOLD 지속 forward)을 score_cell로 채점·verdict 축적. `minority_report/hold_advisor_exit_lessons.py` + `tools/run_hold_advisor_exit_validation.py` + 세션마감 hook(토글 게이트). 토글 `HOLD_ADVISOR_EXIT_LESSON_ENABLED=false`(기본). **남은 운영**: ① forward 3거래일 성숙 대기(insufficient→valid) ② v2 market_regime full sync 후 collect 재실행(현재 `unknown` 폴백) ③ verdict 보고 토글 ON 판정. |
