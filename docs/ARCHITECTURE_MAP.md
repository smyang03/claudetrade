...
| 전략 | `strategy/` | gap, momentum, mean reversion, adaptive params | [Intraday Strategy Roadmap](plans/PLAN_intraday_strategy_roadmap.md) |
| PathB | `runtime/`, `decision/`, `data/v2_event_store.db` | Claude price plan 기반 live runtime | [Dual Runtime Architecture](plans/DUAL_RUNTIME_ARCHITECTURE.md), [PathB Live Plan](../pathb_v2_live_plan.md) |
| 브로커/KIS | `kis_api.py`, `runtime/broker_truth_snapshot.py` | 주문, 잔고, 체결, broker truth | [KIS_API_TODO.md](KIS_API_TODO.md), [KIS_WS_FILL_SYNC_PLAN.md](KIS_WS_FILL_SYNC_PLAN.md) |
...
| `data/v2_event_store.db` | PathB/V2 event store | [Dual Runtime Architecture](plans/DUAL_RUNTIME_ARCHITECTURE.md) |
...
docs/
  ...
  TODO_ROADMAP.md                   # active priority report
...
- `docs/plans/`는 미완료 계획과 active follow-up만 둡니다.
- 완료된 plan은 `docs/plans/`에서 삭제하고, 요약은 `DEVELOPED_WORK.md`에 둡니다.
