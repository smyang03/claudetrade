...
from tools.analyze_counterfactual_paths import analyze_counterfactual_paths
...
def test_retry_missing_targets_only_price_pending() -> None:
    with tempfile.TemporaryDirectory() as tmp:
        root = Path(tmp)
        db = root / "candidate_audit.db"
        price_root = root / "price"
...
def test_update_counterfactual_outcomes_skips_non_immediate_rows() -> None:
    with tempfile.TemporaryDirectory() as tmp:
        root = Path(tmp)
        db = root / "candidate_audit.db"
        price_root = root / "price"
