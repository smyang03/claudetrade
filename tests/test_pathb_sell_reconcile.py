...
class PathBSellReconcileTests(unittest.TestCase):
    ...
    def test_exit_order_unknown_still_held_without_sell_evidence_recovers_to_filled(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertTrue(run["plan"]["stale_exit_order_unconfirmed"])
            self.assertTrue(run["plan"]["sellable_qty_observation_required"])
            ...
            self.assertTrue(pos["stale_exit_order_unconfirmed"])
            self.assertEqual(pos["stale_exit_execution_id"], "sell1")
            self.assertTrue(pos["sellable_qty_observation_required"])
            self.assertEqual(pos["pathb_sell_state"], "stale_exit_order_recovered_as_still_held")
    ...
    def test_pending_sell_ignores_exact_order_fill_before_entry(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            runtime, plan = _runtime(
                tmp,
                balance_provider=lambda market, force: {"cash": 0, "stocks": []},
