...
- 재호가 상한 = min(limit×(1+max_chase%), target×(1−min_reward%), buy_zone_high). 셋 중 낮은 값.
- 현재가가 상한 위로 이미 튀었으면 재호가 안 함(MISS) — 추격 손실 차단.
- target/limit 비정상이면 보수적(None).

2026-06-26(실측 결정): planned buy_zone_high 위 추격 진입 18건이 net −1.66% PF 0.10(시스템
최악 버킷). chase_ceiling=limit×(1+chase%)가 zone 위를 허용했고 다중 requote가 천장을 누적
상승시켰다. → buy_zone_high를 하드 상한으로 추가. limit이 어떻게 변하든 planned zone 위로는
절대 재호가/체결하지 않는다(zone 안 데드존 보완 이점은 유지). enforce.
"""
...
def requote_decision(
    *,
    ...
    if buy_zone_high and float(buy_zone_high) > 0:
        # planned buy_zone 위로는 절대 추격 안 함(실측: zone 위 진입 net −1.66% PF 0.10).
        # limit이 다중 requote로 올라가도 원래 zone_high를 하드 상한으로 고정.
        requote_cap = min(requote_cap, float(buy_zone_high))
