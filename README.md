...
| 항목 | 내용 |
|------|------|
| **US 잔고 조회 제한** | `get_balance(market='US')` 구현됨. 다만 통합계좌 구조상 총자산 기준은 `KR 잔고조회 총액`을 우선 사용 |
| **US 실계좌 주의** | KIS 해외주식 주문/조회 연동됨. 단, 거래소 코드 매핑이 없는 종목은 주문 전에 매핑 추가 필요 |
...
브라우저에서 `http://localhost:5000` 접속. 봇과 별도 프로세스로 실행.

> **주의**: 대시보드는 `.env` 파일을 자동으로 로드한다. `PAPER_CASH` 등 설정이 반영되지 않으면 `.env`가 프로젝트 루트에 있는지 확인.

| URL | 내용 |
|-----|------|
| `/` | 오늘 현황 (요약 카드, 분석가 판단, 자산 곡선) |
| `/history` | 기간별 성과 (이번주/이번달/3개월/전체, 월별 테이블) |
| `/trades` | 매매 원장 (날짜·종목·전략·매수-매도 필터) |
| `/analytics` | 분석 (분석가 적중률, 모드별·전략별 성과) |
