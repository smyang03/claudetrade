        ...
        rows.append(
            {
                ...
                # 외국인 단독 수급 관측 피처(shadow-only, foreign_flow_features 산출) — forward 승격 판단용 원장
                "foreign_flow_window_count": _safe_int(feature_candidate.get("foreign_flow_window_count")),
                "foreign_flow_buy_days_consec": _safe_int(feature_candidate.get("foreign_flow_buy_days_consec")),
                "foreign_flow_sell_days_consec": _safe_int(feature_candidate.get("foreign_flow_sell_days_consec")),
                "foreign_flow_net_qty_5d": _safe_float(feature_candidate.get("foreign_flow_net_qty_5d")),
                ...
                "usable_rows": int(candidate.get("usable_rows") or candidate.get("history_usable_rows") or 0),
                "required_rows": int(candidate.get("required_rows") or candidate.get("history_required_rows") or 0),
                "first_failed_at": str(candidate.get("first_failed_at") or ""),
                "last_failed_at": str(candidate.get("last_failed_at") or ""),
                "failure_count": int(candidate.get("failure_count") or 0),
                "cooldown_until": str(candidate.get("cooldown_until") or ""),
