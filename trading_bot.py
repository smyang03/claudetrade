...
class TradingBot(MarketUtilsMixin, StateMixin):
    ...
    def _entry_timing_signal_fired(self, market: str, ticker: str, *, price: float, strategy: str, reason: str = "") -> dict:
        ...
        if tracker is None:
            return {}
        try:
            return tracker.mark_signal_fired(
                market,
                ticker,
                session_date=self._current_session_date_str(market),
                price=price,
    ...
    def _entry_timing_order_sent(self, market: str, ticker: str, *, price: float, order_no: str = "", strategy: str = "", qty: int = 0) -> dict:
        ...
        if tracker is None:
            return {}
        try:
            return tracker.mark_order_sent(
                market,
                ticker,
                session_date=self._current_session_date_str(market),
                price=price,
    ...
    def _build_lesson_candidates(self, market: str, ops_review_snapshot: dict) -> list[dict]:
        ...
        _append(
            ...
            "selection",
            "prompt_hint",
            "trade_ready 대비 실제 신호 전환율이 낮습니다. trade_ready 승격 기준을 보수적으로 재검토하세요.",
        )
        _append(
            ...
            "selection",
            "prompt_hint",
            "watch_only에서 놓친 상승 비율이 높습니다. watch_only 유지 기준과 veto 품질을 재검토하세요.",
        )
        _append(
            ...
            "strategy",
            "strategy_review",
            "continuation 성과가 약합니다. live 재진입보다는 shadow 관찰을 우선하세요.",
        )
        _append(
            ...
            "consensus",
            "logic_review",
            "전원합의와 최종합의 불일치가 감지됩니다. unanimous override 경로를 점검하세요.",
        )
        ...
        if affordability_events:
            candidates.append({
                ...
                "scope": "execution",
                "action": "prompt_hint",
                "summary": "실행 가능 금액/수량 부족 사례가 반복됐습니다. affordability fail 이유를 selection/entry 리뷰에 반영하세요.",
    ...
    def _execute_sell(self, cand: dict, market: str, reason: str,
        ...
        if reason in ("loss_cap", "profit_floor", "stop_loss", "trail_stop"):
    ...
    def _entry_scan_interval_sec(self, market: str) -> int:
        ...
        return _ENTRY_SCAN_REGULAR_INTERVAL_MIN * 60       # KR 정규: 5분
    ...
    def session_close(self, market: str):
        ...
        log.info(f"[lesson candidates {market}] {len(lesson_candidates)}개")
