...
from bot.state import StateMixin, _atomic_json_dump
...
def _read_bot_pid_file(path: Path) -> dict:
    try:
...
def _write_bot_pid_file(is_paper: bool = True):
    _mode = "paper" if is_paper else "live"
    _pid_file = _bot_pid_file(is_paper)
    payload = json.dumps(
        {
            "pid": os.getpid(),
            "mode": _mode,
    ...
    for _attempt in range(2):
        try:
            ...
            if _check_duplicate_bot(is_paper):
                raise RuntimeError(f"{_mode} bot is already running")
            try:
                _pid_file.unlink()
            except FileNotFoundError:
                pass
            except Exception as exc:
                raise RuntimeError(f"failed to clear stale pid lock: {exc}") from exc
    raise RuntimeError(f"failed to acquire {_mode} pid lock")
def _clear_bot_pid_file(is_paper: bool = True):
    try:
        _pid_file = _bot_pid_file(is_paper)
        if not _pid_file.exists():
            return
        data = _read_bot_pid_file(_pid_file)
...
def _mode_family(mode: str) -> str:
    ...
    if normalized in {"AGGRESSIVE", "MODERATE_BULL", "MILD_BULL"}:
        return "RISK_ON"
    if not normalized or normalized in {"BALANCED", "CAUTIOUS", "NEUTRAL"}:
        return "BALANCED"
    return "RISK_OFF"
...
class TradingBot(MarketUtilsMixin, StateMixin):
    def __init__(self, is_paper: bool = True):
        ...
        _bal_retry_delay = 5
        bal_kr = None
        if not self._market_enabled("KR"):
            bal_kr = {"cash": 0, "total_eval": 0, "stocks": []}
            _log_normal("info", "KR disabled by ENABLED_MARKETS; skipping KR balance lookup")
        for _attempt in (range(1, _bal_retry_max + 1) if bal_kr is None else []):
            try:
                ...
                if _attempt < _bal_retry_max:
                    ...
                    elif self._market_enabled("US"):
                        reason = f"KIS KR startup balance failed after {_bal_retry_max} attempts: {e}"
                        self.enabled_markets.discard("KR")
                        self.startup_disabled_markets["KR"] = reason
                        _log_risk("warning", f"{reason}; continuing with US only")
                        bal_kr = {"cash": 0, "total_eval": 0, "stocks": []}
                    else:
        ...
        if init_cash <= 0:
            if is_paper:
                ...
                if self._market_enabled("US"):
                    reason = "KIS KR startup balance is zero"
                    self.enabled_markets.discard("KR")
                    self.startup_disabled_markets["KR"] = reason
                    _log_risk("warning", f"{reason}; continuing with US only")
                    init_cash = 0
        ...
        if self._market_enabled("US"):
            try:
                ...
                if not is_paper:
                    self.enabled_markets.discard("US")
                    self.startup_disabled_markets["US"] = f"KIS US startup balance failed: {e}"
        else:
            _log_normal("info", "US disabled by ENABLED_MARKETS; skipping US balance lookup")
        init_cash_total = init_cash + us_cash_init_krw
        if init_cash_total <= 0:
            if is_paper:
                init_cash_total = int(os.getenv("PAPER_CASH", "10000000"))
            else:
                raise SystemExit("No enabled market startup balance available")
        ...
        if self.runtime_config.get_bool("ENABLE_EFFECTIVE_CONFIG_SNAPSHOT", True):
            try:
                self._runtime_config_snapshot_path = str(self.runtime_config.write_redacted_snapshot(label=self._mode))
                log.info(f"[config snapshot] {self._runtime_config_snapshot_path}")
            except Exception as exc:
                log.warning(f"[config snapshot failed] {exc}")
        ...
        self._post_open_price_history: dict[str, list[dict]] = {}
        ...
        for _market, _reason in getattr(self, "startup_disabled_markets", {}).items():
            self._set_broker_state(
                _market,
                trust_level="untrusted",
                error=f"startup_disabled: {_reason}",
            )
    ...
    def _sync_runtime_with_broker(self):
        ...
        if self._market_enabled("KR"):
            try:
                ...
                try:
                    ...
                    if not self._market_enabled("US"):
                        return
            except Exception as e:
                _kr_fails = self._broker_state.get("KR", {}).get("consecutive_failures", 0) + 1
                _kr_level = "untrusted" if _kr_fails >= 3 else "degraded"
            ...
            self._set_broker_state("KR", trust_level="untrusted", error="market_disabled")
        broker_us: dict = {}
        us_ok = False
        if self._market_enabled("US"):
            try:
        ...
        for pos in self.risk.positions:
            ...
            if market == "KR" and not self._market_enabled("KR"):
    ...
    def _lookup_ticker_name(self, ticker: str, market: str) -> str:
        ...
        try:
            ...
            pass
        return ""
    def _local_filled_qty_for_order(self, market: str, ticker: str, order_no: str) -> int:
        ...
        for pos in list(getattr(self.risk, "positions", []) or []):
            ...
            if not pos_ticker:
                continue
            pos_market = self._ticker_market(pos_ticker)
            pos_key = pos_ticker.upper() if pos_market == "US" else pos_ticker
            if pos_market != market_key or pos_key != ticker_key:
                continue
            pos_order_no = str(pos.get("order_no", "") or "").strip()
            pos_execution_id = str(pos.get("v2_execution_id", "") or "").strip()
            if pos_order_no == order_no or pos_execution_id == order_no:
                total += int(pos.get("qty", 0) or 0)
        return total
    def _reconcile_pending_orders(self, broker_kr: dict, broker_us: dict):
        ...
        for order in self.pending_orders:
            ...
            already_filled_qty = max(
                int(order.get("filled_qty_accum", 0) or 0),
                self._local_filled_qty_for_order(market, ticker, order_no),
            )
            fill = None
            if not order.get("filled_price_native") or order.get("partial_fill_at"):
                try:
                    fill = (
                        get_order_fill_kr(
            ...
            query_new_qty = max(0, query_filled_qty - already_filled_qty)
            filled_by_broker = bool(broker_pos and broker_qty >= current_qty + order_qty)
            filled_by_query = bool(fill and query_new_qty >= max(order_qty, 1))
            partial_qty = 0
            if 0 < query_new_qty < order_qty:
                partial_qty = query_new_qty
    ...
    def _native_position_price(self, pos: dict, market: str, *, current: bool) -> float:
        if str(market or "").upper() == "US":
            ...
            native = self._float_or_zero(pos.get(key))
            if native > 0:
                return native
            fallback_key = "current_price" if current else "entry"
            fallback = self._float_or_zero(pos.get(fallback_key))
    ...
    def _position_with_latest_price_context(self, pos: dict, market: str) -> dict:
        ...
        if market_key == "US":
            keys = [ticker]
            if ticker.upper() not in keys:
                keys.append(ticker.upper())
            raw_cached = 0.0
            krw_cached = 0.0
            for key in keys:
                raw_cached = self._float_or_zero(getattr(self, "price_cache_raw", {}).get(key))
                if raw_cached > 0:
                    break
            for key in keys:
                krw_cached = self._float_or_zero(getattr(self, "price_cache", {}).get(key))
                if krw_cached > 0:
                    break
            fx = self._float_or_zero(getattr(self, "usd_krw_rate", 0))
            if raw_cached > 0:
                price_pos["display_current_price"] = raw_cached
                if fx > 0:
                    price_pos["current_price"] = raw_cached * fx
            elif krw_cached > 0:
                price_pos["current_price"] = krw_cached
                if fx > 0:
                    price_pos["display_current_price"] = krw_cached / fx
            return price_pos
    ...
    def _execute_sell(self, cand: dict, market: str, reason: str,
        ...
        audit_keys = (
            ...
            "auto_sell_review_cooldown_until",
            "pathb_reference_target",
            "pathb_reference_stop",
            "pathb_reference_confidence",
            "pathb_reference_status",
    ...
    def session_open(self, market: str, trigger: str = "schedule"):
        ...
        if self._current_judgment_phase(market) == _JUDGMENT_PHASE_PREOPEN:
            ...
            # Reset before WS ticks can repopulate session-scoped live state.
        ...
        if live_path.exists():
            try:
                ...
                if saved_mode not in ("historical_sim",) and saved.get("judgments") and saved.get("consensus"):
                    ...
                    restored_meta = self._normalize_selection_meta_runtime(
                        market,
                        restored_meta,
                        saved.get("tickers", []),
                        mode=saved.get("consensus", {}).get("mode", ""),
                    )
                    self.selection_meta[market] = self._enforce_trade_ready_price_targets(market, restored_meta)
    ...
    def run_cycle(self, market: str):
        if not self._enter_market_task(market, "run_cycle"):
            return
        _cycle_started_at = time.perf_counter()
        if not self.session_active:
            self._record_cycle_latency(market, _cycle_started_at, stage="run_cycle", reason="session_inactive")
        ...
        if self._check_market_halt(
            ...
            self._record_cycle_latency(market, _cycle_started_at, stage="run_cycle", reason="session_halt")
            self._leave_market_task(market, "run_cycle")
            return
        ...
        if _since_open < _guard_sec:
            ...
            self._record_cycle_latency(market, _cycle_started_at, stage="run_cycle", reason="startup_guard")
            self._leave_market_task(market, "run_cycle")
            return
        # ── 장전 SELL 예약 큐 처리 ─────────────────────────────────────────
        ...
        if market == "KR":
            ...
            log.debug(f"[KR 전략] 투표={_voted_strat} 시도순서={_kr_strat_list}")
        else:
            _kr_strat_list = []
            ...
            try:
                ...
                if _prev_price > 0 and abs(price - _prev_price) / _prev_price > 0.30:
                    ...
                    if _outlier_confirmed:
                        ...
                        continue
                # US는 WS 시세 미수신/실전 WS 수신 지연 시 scan polling 가격으로 OR 근사 추적
                self.price_cache_raw[ticker] = price
                self.price_cache[ticker] = risk_price
                self.risk.update_prices(self.price_cache, self.price_cache_raw)
                _po_meta_for_feature = _preopen_meta_by_ticker.get(ticker, {})
                self._maybe_record_post_open_feature_snapshot(
                    market,
                    ticker,
                    price,
                    price_info=price_info,
                    preopen_meta=_po_meta_for_feature,
                    source="run_cycle",
                )
                # US는 WS 시세 미수신/실전 WS 수신 지연 시 scan polling 가격으로 OR 근사 추적
                ...
                if not _entry_judgment_ok:
                    ...
                    _raw_trade_ready_skip = self.trade_ready_tickers.get(market, []) or []
                    _trade_ready_skip_set = (
                        {str(t).upper() for t in _raw_trade_ready_skip}
                        if market == "US"
                    ...
                    self._record_judgment_gate_event(
                        ...
                        judgment_gate_reason=_entry_judgment_block,
                        price=float(price),
                        mode=mode,
                        extra={
                            "preopen_rank": _po_meta.get("shadow_preopen_rank"),
                ...
                # price_collector가 실제 일봉을 CSV에 반영하면 다음 TTL 갱신 시 실제 봉으로 교체된다.
                # ── 장중 임시 당일봉 주입 (KR/US 공통) ───────────────────
                        ...
                        if _is_single_price and not _has_gap:
                            ...
                            if _vol_missing:
                                ...
                                log.debug(
                                    ...
                                    f" -> hist_avg={_hist_avg:,.0f} x prog={_sess_prog:.2f}"
                                    f" = {_fallback_vol:,.0f}"
                                )
                ...
                # KR momentum: 기본 cap 초과 구간은 단계형 감액으로 받고,
                # high cap 초과 구간만 차단한다.
                ...
                if _entry_judgment_block == "ok_unconfirmed" and effective_size > _UNCONFIRMED_ENTRY_SIZE_CAP_PCT:
                    _before_unconfirmed_cap = int(effective_size)
                    effective_size, _unconfirmed_cap_applied = self._apply_unconfirmed_phase_size_cap(
                        market,
                        ticker,
                        effective_size,
                        judgment_gate_reason=_entry_judgment_block,
        ...
        # ── 수집된 신호 정렬 후 주문 실행 ───────────────────────────────────
                ...
                log.error(f"cycle error [{ticker}]: {e}", exc_info=True)
        # ── 수집된 신호 정렬 후 주문 실행 ───────────────────────────────────
        if _pending_signals:
                ...
                try:
                    ...
                    if is_too_small:
                        ...
                        log.info(
                            ...
                            f"({order_cost:,.0f}원 < {min_effective_order:,.0f}원, "
                            f"budget={order_budget:,.0f}원) - skip"
                        )
                        continue
                    if qty <= 0:
                        ...
                        if _ML_DB_ENABLED:
                            _ml_write_eval(_s_tk, _s_px, _s_row, "SKIPPED",
                                           block_reason_=_qty_zero_reason,
                                           diag_json_={"reason": _qty_zero_reason, **affordability_diag})
                        tsdb.update_signal(_tsdb_id, _s_strat, _s_ep, _sig_at, _qty_zero_reason)
                    ...
                    try:
                        ...
                        if _t2_small:
                            ...
                            log.info(
                                ...
                                f"({_t2_cost:,.0f}원 < {_t2_min:,.0f}원, budget={_t2_budget:,.0f}원) - skip"
                            )
                            continue
                        if _t2_qty <= 0:
                            _t2_zero_reason = str(_t2_affordability.get("affordability_reason") or "qty_zero")
                            _t2_zero_reason = _t2_zero_reason if _t2_zero_reason else "qty_zero"
                            self._bump_runtime_reason("US", _t2_ticker, "affordability_fail")
                            self._bump_runtime_reason("US", _t2_ticker, _t2_zero_reason)
                            self._record_decision_event(
                                "US",
                                "buy_skipped",
                                _t2_ticker,
                            ...
                            analysis_log.info(
                                f"[skip US] {_t2_ticker} {_t2_zero_reason}",
                                extra={"extra": {
                                    "event": "entry_skip", "market": "US", "ticker": _t2_ticker,
                                    "reason": _t2_zero_reason, "strategy": "sector_play",
                                    "mode": mode, "price": _t2_price, "risk_price_krw": _t2_risk_price,
                                    "qty": int(_t2_qty), "order_cost_krw": float(_t2_cost),
                                    **_t2_affordability,
                                }},
                ...
                for _play in _kr_t2_plays:
                    ...
                    try:
                        ...
                        if _t2_qty <= 0:
                            ...
                            self._bump_runtime_reason("KR", _t2_ticker, _t2_zero_reason)
                            self._record_decision_event(
                                "KR",
                                "buy_skipped",
                                _t2_ticker,
                            ...
                            analysis_log.info(
                                f"[skip KR] {_t2_ticker} {_t2_zero_reason}",
                                extra={"extra": {
                                    "event": "entry_skip", "market": "KR", "ticker": _t2_ticker,
    ...
    def _record_decision_event(self, market: str, action: str, ticker: str, **kwargs):
        ...
        event = {
            ...
            "auto_sell_review_detail": kwargs.get("auto_sell_review_detail", ""),
            ...
            "auto_sell_review_cooldown_until": kwargs.get("auto_sell_review_cooldown_until", ""),
        ...
        if action not in _SKIP_ACTIONS:
            ...
            # closed: 종료 요약
                ...
                record.update({
                    ...
                    "auto_sell_review_action": event.get("auto_sell_review_action", ""),
                    "auto_sell_review_detail": event.get("auto_sell_review_detail", ""),
                    "auto_sell_review_confidence": event.get("auto_sell_review_confidence", 0),
                    "auto_sell_review_fallback": event.get("auto_sell_review_fallback", False),
                    "auto_sell_review_cooldown_until": event.get("auto_sell_review_cooldown_until", ""),
            ...
            elif action == "auto_sell_review":
                record.update({
                    "qty": event["qty"],
                    "price_native": event["price_native"],
                    "price_krw": event["price_krw"],
                    "auto_sell_review_reason": event.get("auto_sell_review_reason") or event.get("reason", ""),
                    "auto_sell_review_action": event.get("auto_sell_review_action", ""),
                    "auto_sell_review_detail": event.get("auto_sell_review_detail") or event.get("detail", ""),
                    "auto_sell_review_confidence": event.get("auto_sell_review_confidence", 0),
                    "auto_sell_review_fallback": event.get("auto_sell_review_fallback", False),
                    "auto_sell_review_cooldown_until": event.get("auto_sell_review_cooldown_until", ""),
                })
    ...
    def _maybe_push_dashboard(self, force: bool = False):
        ...
        realized_excluding_eval_pnl_krw = float(self._market_realized_pnl_krw(market) or 0.0)
        if abs(realized_excluding_eval_pnl_krw) < 1e-9 and abs(float(self.risk.daily_pnl or 0.0)) > 0:
            realized_excluding_eval_pnl_krw = float(self.risk.daily_pnl or 0.0)
        unrealized_today_delta_krw = float(self._market_unrealized_session_pnl_krw(market) or 0.0)
        trading_pnl_krw = realized_excluding_eval_pnl_krw + unrealized_today_delta_krw
        base_equity = float(self._market_session_start_equity_krw(market) or 0.0)
        pnl_pct = (trading_pnl_krw / base_equity * 100.0) if base_equity > 0 else self._daily_pnl_pct(market)
        pnl_krw = int(round(trading_pnl_krw))
        market_positions = [
    ...
    def session_close(self, market: str):
        ...
        market_change = self._get_market_change_pct(market)
        secondary_market_change = self._get_secondary_change_pct(market)
        actual = {
            "market_change": market_change,
            "secondary_market_change": secondary_market_change,
...
def _check_duplicate_bot(is_paper: bool = True) -> bool:
    ...
    try:
        ...
        if existing_pid and existing_pid != os.getpid() and _process_alive(existing_pid):
            return True
        try:
            _pid_file.unlink()
        except FileNotFoundError:
...
def main(is_paper: bool = True):
    ...
    try:
        ...
        log.error(f"[duplicate bot blocked] {_mode} pid lock failed: {exc}")
        sys.exit(1)
