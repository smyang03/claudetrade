...
• "변동성 프록시: KOSPI 실현변동성 20d 연율 X%"가 함께 오면 변동성 수준 판단에 사용:
  X<15% 저변동, 15~25% 보통, >25% 고변동. 프록시는 VKOSPI(내재변동성)가 아니라 실현변동성임 — 급변 당일에는 후행함을 감안. 프록시가 있으면 "VKOSPI 결측"을 불확실성 사유로 반복 인용하지 말 것.
