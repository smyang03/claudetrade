...
# 실측 prior — 이 계좌 2026-05~06 청산 원장 재채점에서 도출.
# 2026-06-23 정직화: 기존 단일 수치(-2.36%p / profit_pullback -8.91%p "최악")는
# 최악 서브셋 체리픽 + mfe NULL 오염기간(6/19 배선 fix 전) + 5월 강세장 표본 집중이라
# 클린 재측정에서 재현되지 않았다. 과장된 절대값을 제거하고 방향(반납 경향)·표본 한계·
# 승자 조기절단 방지를 함께 명시한다. 효과는 HOLD_ADVISOR_PROFIT_GUARD를 forward-validation에
# 편입해 라이브 net으로 검증한다(미달 시 토글 false). 판단을 하드 차단하지 않는다.
_PROFIT_GUARD_PRIOR = (
    "\n- 이익 중(pnl_pct>0)의 HOLD는 고점 대비 이익의 상당 부분을 반납하는 경향이 있다(약 70%가 일부 반납).\n"
    "  다만 이 기저율은 2026-05 강세장 표본에 집중되어 있고, 최종 실현은 대부분 양수로 마감했다(승자 다수).\n"
    "  목표가 부근이거나 모멘텀이 꺾이면(거래량 둔화·고점 대비 하락) HOLD로 더 먹으려 하지 말고 SELL(익절)을 우선한다.\n"
    "  단 추세가 살아있고 고점 갱신 중이면 HOLD 유지(러너는 끝까지) — 대부분이 양수로 마감하므로 강한 러너를 조기 절단하지 않는다."
)
...
- advisor_context_v2.portfolio_context가 있으면 같은 베타(메가캡/동일 테마) 동시 보유 수를 확인한다.
  동질 포지션이 이미 다수 보유 중이면 이 포지션의 HOLD는 집중 리스크를 가중해 평가한다."""
    + (_PROFIT_GUARD_PRIOR if os.getenv("HOLD_ADVISOR_PROFIT_GUARD_ENABLED", "true").lower() == "true" else ""))
