...
| 모의투자 데이터가 실거래 학습에 혼입 | `bot_mode` 컬럼(ticker_selection_db/intraday_strategy_db), `is_simulated`/`data_source` (decisions.db), `{mode}_api_usage.json` 분리로 방어 |
...
로그는 가능하면 아래 3개 채널로 나눠 기록한다.

- `logs/normal/`
  - 정상 상태, 기준값 확정, 완료 이벤트
  - 예: 잔고 초기화, session baseline 확정, 환율 갱신 성공, 세션 완료

- `logs/risk/`
  - 위험, 경고, 회계 이상, 동기화 불신, halt, 실패
  - 예: 브로커 동기화 실패, false halt 의심, 데이터 무결성 경고, 주문 실패, fallback 진입

- `logs/flow/`
  - 세션 시작/종료, cycle 스킵, 큐 처리, 작업 경로
  - 예: `session_open`, `session_close`, startup guard, 장전 SELL 큐 소비, 작업 진입/이탈

원칙:
- 기존 `logs/system/`은 유지한다
- 새로 추가하는 로그나 수정하는 구간의 로그는 가능하면 `normal / risk / flow`에도 함께 남긴다
- 과거 로그를 전부 일괄 변환하지는 않는다. 작업하는 범위 안에서만 점진적으로 맞춘다
...
운영 이슈나 장중 이상 징후를 분석할 때는 아래 순서로 본다.

1. **`logs/risk/`부터 확인한다**
   - halt, 동기화 실패, 회계 이상, fallback, 주문 실패를 먼저 본다

2. **그다음 `logs/flow/`를 본다**
   - 세션이 실제로 어떤 경로를 탔는지, 스킵/보류/큐 소비가 있었는지 확인한다

3. **마지막으로 `logs/normal/`과 `logs/system/`을 본다**
   - 정상 완료 이벤트와 전체 흐름을 보며 시간축을 맞춘다

즉, 오늘 로그 분석을 시작할 때는 항상 **위험/경고 로그부터 먼저 본다**.
...
```bash
# 봇 실행 — 모의투자 (env.paper 자동 로드)
python trading_bot.py --paper

# 봇 실행 — 실거래 (env.live 자동 로드)
python trading_bot.py --live

# paper/live 동시 운용 (별도 터미널)
python trading_bot.py --paper   # 터미널 1 (KIS_IS_PAPER=true)
python trading_bot.py --live    # 터미널 2 (KIS_IS_PAPER=false)
...
### SharedJudgmentCache — paper/live 간 판단 공유

`shared_judgment_cache.py`가 paper/live 두 프로세스 사이에서 아침 Claude 판단(3인 합의)을 공유한다.

- 저장 경로: `state/shared_judgment_{MARKET}_{YYYYMMDD}.json`
- **먼저 실행된 프로세스**가 Claude를 호출하고 결과를 저장 → **나중 프로세스**는 캐시를 읽어 재사용
- CACHE_VERSION=2로 버전 검증. VIX 보정과 스크리닝은 항상 각 프로세스가 독자 실행
- tickers/universe_tickers는 저장하지 않음 (런타임 재생성)
- 공유 캐시가 없거나 날짜/버전 불일치 시 Claude 새로 호출
- 수동 초기화: 해당 `state/shared_judgment_*.json` 삭제 후 두 프로세스 재시작
...
data/ml/decisions.db                    ← 매 사이클 write_decision(), 청산 시 update_exit()
                                           ...
                                           is_simulated/data_source 컬럼으로 paper/live 구분
data/ticker_selection_log.db            ← Claude 종목 선택 이력, bot_mode 컬럼으로 paper/live 구분
data/intraday_strategy_log.db           ← OR/VWAP 전략 probe/signal/trade/outcome 로그
state/paper_open_positions.json         ← 모의투자 포지션 (runtime, git 추적 제외)
state/live_open_positions.json          ← 실전 포지션 (runtime, git 추적 제외)
state/shared_judgment_{market}_{date}.json ← paper/live 간 아침 Claude 판단 공유
state/{mode}_api_usage.json             ← Anthropic API 토큰/비용 누적 (mode별 분리)
...
| `credit_tracker.py` | Anthropic API 토큰/비용 누적 (`{mode}_api_usage.json` paper/live 분리) |
| `shared_judgment_cache.py` | paper/live 간 아침 Claude 판단 공유 (FileLock, CACHE_VERSION=2) |
| `ticker_selection_db.py` | Claude 종목 선택 이력 (`bot_mode` 컬럼으로 paper/live 구분) |
| `intraday_strategy_db.py` | OR/VWAP 전략 raw 데이터 로그 (`bot_mode` 컬럼으로 paper/live 구분) |
...
- [x] **Dual Runtime Phase 0** — paper/live 프로세스 2개 동시 운용 구조 완성
  - 상태 파일 경로 paper_/live_ prefix 분리 (`bot/state.py`)
  - `.env.paper` / `.env.live` 분리, `--paper` / `--live` 플래그
  - `shared_judgment_cache.py` — 두 프로세스 간 아침 Claude 판단 공유
  - `telegram_reporter.py` — `[모의]`/`[실전]` 모드 라벨 추가 (10개 함수)
  - DB 4종 paper/live 분리: `bot_mode` 컬럼, `{mode}_api_usage.json`, `is_simulated`/`data_source`
  → 상세 계획: [`docs/plans/DUAL_RUNTIME_ARCHITECTURE.md`](docs/plans/DUAL_RUNTIME_ARCHITECTURE.md)

### 예정

- [ ] **trading_bot.py 모듈화** — 단일 파일을 13개 모듈로 분리 (토큰 효율 + 유지보수)
...
### Dual Runtime Phase 0 — 운용 중

paper/live 두 프로세스를 동시에 별도 터미널에서 실행. 아침 Claude 판단은 `shared_judgment_cache.py`로 공유. 계좌/포지션/손익/halt는 완전 분리.

```
python trading_bot.py --paper   # 모의투자 (.env.paper 로드)
python trading_bot.py --live    # 실거래 (.env.live 로드, MAX_ORDER_KRW 소액 설정)
```
