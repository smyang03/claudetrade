...
6. **정상 동작하는 코드는 임의로 수정하지 않는다**
   요청받은 기능 외의 코드를 "개선"하거나 "정리"하지 않는다. 건드려야 할 이유가 생기면 먼저 개발자에게 물어본다.
   리팩토링·최적화·스타일 수정은 명시적으로 요청받은 경우에만 한다.

7. **기능 업데이트 후에는 반드시 검증 단계를 거친다**
   새 기능을 추가하거나 기존 동작을 변경한 뒤에는 아래 순서로 검증한다:
   ```bash
   # 1. 구문 검사
   python -c "import ast; ast.parse(open('trading_bot.py', encoding='utf-8').read()); print('OK')"
   # 2. 관련 단위 테스트
   python -m unittest test_broker_sync_cash.py -v
   # 3. 변경이 데이터 흐름(로그·DB 기록)에 영향을 주는지 확인
   # 4. git diff --stat 으로 의도하지 않은 파일이 포함되지 않았는지 확인
   ```
   검증 없이 "동작할 것 같다"는 가정으로 커밋하지 않는다.

10. **.env.example에 환경변수를 추가할 때, 코드에서 실제로 읽는지 먼저 확인한다**
...
- [ ] **trading_bot.py 모듈화** — 4000줄 단일 파일을 역할별로 분리 (토큰 효율 + 유지보수)
